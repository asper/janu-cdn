...
use std::path::Path;
use janu::{Path as ZPath, Selector};
use janu::{ZError, ZErrorKind, ZResult, Janu};
...
impl Client {
    pub fn new(z: Arc<Janu>) -> Self {
        Self { z }
    }

    /// Uploads a file to Janu-CDN.
    ///
    pub async fn upload(&self, file_path: &Path, resource_name: &ZPath) -> ZResult<()> {
        ...
        let value = janu::Value::Json(data);
    ...
    pub async fn download(&self, resource_name: &ZPath, destination: &Path) -> ZResult<()> {
        ...
        let metadata = {
            ...
            let data = ds.collect::<Vec<janu::Data>>().await;
            ...
            match data.len() {
                ...
                1 => {
                    let kv = &data[0];
                    match &kv.value {
                        janu::Value::Json(value) => Ok(FileMetadata::deserialize(value)?),
            ...
            let data = {
                ...
                match data.len() {
                    ...
                    1 => {
                        let kv = &data[0];
                        match &kv.value {
                            janu::Value::Raw(_, buf) => Ok(buf.to_vec()),
                            _ => zerror!(ZErrorKind::Other {
                                descr: format!(
